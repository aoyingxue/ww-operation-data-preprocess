...
def melt_revenue_for_union(
    ...
    df_melted = df_revenue.melt(
        id_vars=df_revenue.columns[~df_revenue.columns.isin(['确认交付金额','预计交付金额'])],
        value_vars=['确认交付金额','预计交付金额'],
        var_name='交付金额状态',
        value_name='金额',
    )
    df_melted['金额'] = pd.to_numeric(df_melted['金额'])
    df_melted = df_melted.loc[((df_melted['金额']!=0) & ((df_melted['金额']==df_melted['金额']) & (df_melted['金额']!=None))),].copy() # 预计金额为0或空的筛掉
    df_melted['运营状态'] = df_melted[['运营状态', '交付金额状态']].apply(
        lambda x: '已确认' if x['交付金额状态'] == '确认交付金额' else ('非Q-在手' if ((x['运营状态']=='已确认') & (x['交付金额状态']=='预计交付金额')) else x['运营状态']),
        axis=1,
    )
    ## drop unused columns
    df_melted.rename(
    ...
    df_melted.reset_index(drop=True, inplace=True) # reset the index after dropping off columns and rows
    return df_melted
